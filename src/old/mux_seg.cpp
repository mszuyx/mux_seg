...
sensor_msgs::PointCloud2::Ptr camera_cloud_right(new sensor_msgs::PointCloud2 ());
sensor_msgs::PointCloud2::Ptr camera_cloud_left(new sensor_msgs::PointCloud2 ());
...
int MuxSeg::findMean(const pcl::PointCloud<pcl::PointXYZ>::Ptr& pc, double& mean_y, const double percentage){
    int cnt = 0;
    if (percentage>0){
...
bool MuxSeg::checkBlock(const pcl::PointCloud<pcl::PointXYZ>::Ptr& pc, const double z_threshold){
